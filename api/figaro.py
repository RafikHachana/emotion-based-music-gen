...
class Seq2SeqModule(pl.LightningModule):
  ...
  def sample(self, batch,
    ...
    for iter in range(curr_len - 1, max_length):
      i = x.size(1) - 1
      ...
      if x.size(1) >= 10 and (x[:, -5:] == x[:, -10:-5]).all():
        if verbose:
          logging.warning("WARNING: model is repeating itself and producing invalid sequences, removing the invalid tokens")
        x = x[:, :-5]
        bar_ids = bar_ids[:, :-5]
...
def generate_sample_from_description(description, uuid):
    ...
    audio = pm.fluidsynth()
    soundfile.write(f"{uuid}.wav", audio, 44100)
    return f"{uuid}.wav"
